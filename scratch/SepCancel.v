...
Module Type SEP.
  Parameter mprop : Type.
  Parameter lift : Prop -> mprop.
  Parameter star : mprop -> mprop -> mprop.
  Parameter exis : forall A, (A -> mprop) -> mprop.

  Notation "[| P |]" := (lift P).
  Infix "*" := star.
  Notation "'exists' x .. y , p" := (exis (fun x => .. (exis (fun y => p)) ..)).

  Parameters mimp meq : mprop -> mprop -> Prop.

  Infix "===" := meq (no associativity, at level 70).
  Infix "===>" := mimp (no associativity, at level 70).

  Axiom mimp_meq : forall p q, p === q
                               <-> (p ===> q /\ q ===> p).
  Axiom mimp_refl : forall p, p ===> p.
  Axiom mimp_trans : forall p q r, p ===> q -> q ===> r -> p ===> r.
...
Module Make(Import S : SEP).
  Add Parametric Relation : mprop mimp
    reflexivity proved by mimp_refl
    transitivity proved by mimp_trans
    as mimp_rel.

  Lemma meq_refl : forall p, p === p.
  Proof.
    intros; apply mimp_meq; intuition (apply mimp_refl).
  Qed.

  Lemma meq_sym : forall p q, p === q -> q === p.
  Proof.
    intros; apply mimp_meq; apply mimp_meq in H; intuition.
  Qed.

  Lemma meq_trans : forall p q r, p === q -> q === r -> p === r.
  Proof.
    intros; apply mimp_meq; apply mimp_meq in H; apply mimp_meq in H0;
    intuition (eauto using mimp_trans).
  Qed.

  Add Parametric Relation : mprop meq
    reflexivity proved by meq_refl
    symmetry proved by meq_sym
    transitivity proved by meq_trans
    as meq_rel.

  Instance mimp_meq_mor : Proper (meq ==> meq ==> iff) mimp.
  Proof.
    hnf; intros; hnf; intros.
    apply mimp_meq in H; apply mimp_meq in H0.
    intuition eauto using mimp_trans.
  Qed.
  ...
  Instance exis_iff_morphism (A : Type) :
    Proper (pointwise_relation A meq ==> meq) (@exis A).
  Proof.
    hnf; intros; apply mimp_meq; intuition.
    hnf in H.
    apply exis_left; intro.
    eapply exis_right.
    assert (x x0 === y x0) by eauto.
    apply mimp_meq in H0; intuition eauto.
    hnf in H.
    apply exis_left; intro.
    eapply exis_right.
    assert (x x0 === y x0) by eauto.
  ...
  Instance exis_imp_morphism (A : Type) :
    Proper (pointwise_relation A mimp ==> mimp) (@exis A).
  Proof.
    hnf; intros.
    apply exis_left; intro.
    eapply exis_right.
  ...
  Proof.
    intros.
    apply mimp_meq; auto using star_combine_lift1, star_combine_lift2.
  Qed.
  ...
  Theorem star_cancel' : forall p1 p2 q, p1 ===> p2
    -> p1 * q ===> p2 * q.
  Proof.
    intros; apply star_cancel; auto using mimp_refl.
  Qed.
