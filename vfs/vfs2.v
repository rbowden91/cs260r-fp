...
(* M : Monad (XXX) *)
Class fsclass (M : Type -> Type) (vfs : Type) := {
  vnode_: Type;
  VFS_GETROOT: vfs -> M vnode_;
  VFS_SYNC: vfs -> M unit;
  newfs: M unit;
}.
...
(*************************************************************)
(* vfs-level executions *)

Section ops.

Definition vnode: Type := nat. (* XXX placeholder *)
Definition vfs: Type := nat.   (* XXX placeholder *)


Inductive vfscmd: Type :=
| lookup: vnode -> string -> vfscmd
| create: vnode -> string -> vfscmd
| unlink: vnode -> string -> vfscmd
| read: vnode -> nat -> nat -> vfscmd
| write: vnode -> bytes -> nat -> vfscmd
| truncate: vnode -> nat -> vfscmd
| fsync: vnode -> vfscmd
| getroot: vfs -> vfscmd
| sync: vfs -> vfscmd
.

Inductive vfsprogram: Type :=
| prog_cmd: vfscmd -> vfsprogram
| prog_seq: vfsprogram -> vfsprogram -> vfsprogram
| prog_par: vfsprogram -> vfsprogram -> vfsprogram
.

(* a program is made of commands and a trace is made of actions *)

Inductive vfsaction: Type :=
| a_lookup: vnode -> string -> vnode -> vfsaction
| a_create: vnode -> string -> vnode -> vfsaction
| a_read: vnode -> nat -> nat -> bytes -> vfsaction
| a_write: vnode -> bytes -> nat -> vfsaction
| a_truncate: vnode -> nat -> vfsaction
| a_fsync: vnode -> vfsaction
| a_getroot: vfs -> vnode -> vfsaction
| a_sync: vfs -> vfsaction
.

Definition vfstrace := list vfsaction.

Function vfsexec (s : vfsstate) (p : vfsprogram): (vfsstate * vfstrace) :=
   match p with
   | prog_op op => match op with
        | lookup vn name => 
